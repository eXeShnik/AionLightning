...
/// <summary>
/// Port of Java <c>com.aionemu.gameserver.model.DialogAction</c>. Numeric values are load-bearing —
/// they are the raw dialog ids the 4.6 client sends in CM_DIALOG_SELECT — so they must match the
/// Java source exactly. Two Java members intentionally share the same id (57): first-declared wins
/// on lookup via <see cref="DialogActionLookup.FromId"/>, matching Java's linear <c>values()</c> scan.
/// </summary>
...
/// <summary>Id → <see cref="DialogAction"/> lookup, mirroring Java's <c>DialogAction.getActionByDialogId</c>.</summary>
public static class DialogActionLookup
{
    private static readonly Dictionary<int, DialogAction> ById = Build();

    private static Dictionary<int, DialogAction> Build()
    {
        var map = new Dictionary<int, DialogAction>();
        foreach (DialogAction action in Enum.GetValues<DialogAction>())
            map.TryAdd((int)action, action); // first-declared wins on duplicate ids (id 57)
        return map;
    }

    /// <summary>Returns the matching action, or <see cref="DialogAction.NULL"/> when unknown.</summary>
    public static DialogAction FromId(int id) => ById.GetValueOrDefault(id, DialogAction.NULL);
}
