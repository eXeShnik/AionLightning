...
public static class EquipStatsCalculator
{
        ...
        foreach (var item in equippedItems)
        {
            ...
            if (tpl is not null)
            {
                ...
                if (item.EnchantLevel > 0)
                    AccumulateEnchant(item.EnchantLevel, item.Slot, tpl, ref hp, ref pDef, ref pAtk, ref mAtk);
            }
    ...
    private static void AccumulateEnchant(int enchantLvl, int slot, ItemTemplate tpl,
        ref int hp, ref int pDef, ref int pAtk, ref int mAtk)
    {
        if (tpl.IsWeapon)
        {
        ...
        else if (tpl.IsArmor)
        {
            switch (tpl.ArmorTypeName)
            {
                case "ROBE":
                    switch (slot)
                    {
                        case 16: case 32: case 2048: pDef += enchantLvl;     hp += 10 * enchantLvl; break;
                        case 4096:                    pDef += 2 * enchantLvl; hp += 12 * enchantLvl; break;
                        case 8:                       pDef += 3 * enchantLvl; hp += 14 * enchantLvl; break;
                    }
                    break;
                case "LEATHER":
                    switch (slot)
                    {
                        case 16: case 32: case 2048: pDef += 2 * enchantLvl; hp += 8  * enchantLvl; break;
                        case 4096:                    pDef += 3 * enchantLvl; hp += 10 * enchantLvl; break;
                        case 8:                       pDef += 4 * enchantLvl; hp += 12 * enchantLvl; break;
                    }
                    break;
                case "CHAIN":
                    switch (slot)
                    {
                        case 16: case 32: case 2048: pDef += 3 * enchantLvl; hp += 6  * enchantLvl; break;
                        case 4096:                    pDef += 4 * enchantLvl; hp += 8  * enchantLvl; break;
                        case 8:                       pDef += 5 * enchantLvl; hp += 10 * enchantLvl; break;
                    }
                    break;
                case "PLATE":
                    switch (slot)
                    {
                        case 16: case 32: case 2048: pDef += 4 * enchantLvl; hp += 4 * enchantLvl; break;
                        case 4096:                    pDef += 5 * enchantLvl; hp += 6 * enchantLvl; break;
                        case 8:                       pDef += 6 * enchantLvl; hp += 8 * enchantLvl; break;
                    }
                    break;
            }
        }
