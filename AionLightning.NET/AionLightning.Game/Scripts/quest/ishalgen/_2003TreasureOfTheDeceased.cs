// Port of Java data/scripts/system/handlers/quest/ishalgen/_2003TreasureOfTheDeceased.java.
// Talk to Kaldescription NPC 203539, advance to var 1, collect-check → REWARD, turn in.
using System.Linq;
...
public sealed class _2003TreasureOfTheDeceased : QuestHandlerBase
{
    private const int QuestIdConst = 2003;
    private const int NpcId        = 203539;

    private readonly IItemDao _itemDao;

    public _2003TreasureOfTheDeceased(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
        ...
        if (entry.Status == QuestStatus.START)
        {
            switch (dialog)
            {
                ...
                case DialogAction.SELECT_ACTION_1012:
                    await PlayQuestMovieAsync(conn, env.Player, 53, ct);
                    return false;
