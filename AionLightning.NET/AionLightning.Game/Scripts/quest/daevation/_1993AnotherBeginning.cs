// Port of Java data/scripts/system/handlers/quest/daevation/_1993AnotherBeginning.java.
// Talk to Sibylla (203753): EXCHANGE_COIN auto-starts the quest and shows the item-exchange menu;
// picking one of the 25 "old item" dialog pages checks the matching item is held, then a final
// SETPRO-style confirm (10000-10003 -> reward tier 0-3, 10004 -> reward tier 4) flips to REWARD;
// turning in consumes the exchanged item + one Daevanion's Light and grants the chosen tier.
...
public sealed class _1993AnotherBeginning : QuestHandlerBase
{
    private const int QuestIdConst    = 1993;
    private const int Sibylla         = 203753;
    ...
    public _1993AnotherBeginning(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    {
        var npc = engine.RegisterQuestNpc(Sibylla);
    ...
    {
        if (env.TargetId != Sibylla) return false;
