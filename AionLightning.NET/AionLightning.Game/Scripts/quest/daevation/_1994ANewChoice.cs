// Port of Java data/scripts/system/handlers/quest/daevation/_1994ANewChoice.java.
// Talk to Aithra (203754): EXCHANGE_COIN auto-starts the quest and shows the item-exchange menu;
// picking one of the 13 "old item" dialog pages checks the matching item is held, then a final
// confirm (10000-10003 -> reward tier 0-3 shown at pages 5-8, 10004/10005 -> reward tier 4-5 shown
// at pages 41-42) flips to REWARD; turning in consumes the exchanged item + one Daevanion's Light
// and grants the chosen tier. Java already stored the item/reward-tier choice per-player via
// qs.setReward/getReward (a packed int, not an instance field) so there is no shared-mutable bug
// here, but QuestEntry in this port has no equivalent field -- ported using its per-player vars
// instead (var1 = exchanged-item index, var2 = reward tier), which is a straight behavioral match.
using System;
...
public sealed class _1994ANewChoice : QuestHandlerBase
{
    private const int QuestIdConst    = 1994;
    private const int Aithra          = 203754;
    ...
    public _1994ANewChoice(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    {
        var npc = engine.RegisterQuestNpc(Aithra);
    ...
    {
        if (env.TargetId != Aithra) return false;
