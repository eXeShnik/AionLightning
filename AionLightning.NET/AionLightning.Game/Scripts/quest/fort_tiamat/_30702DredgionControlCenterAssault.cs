// Port of Java data/scripts/system/handlers/quest/fort_tiamat/_30702DredgionControlCenterAssault.java (Cheatkiller).
// Accept at 800424 (page 1011, Java sendQuestStartDialog(env, 1073741824) — the packed quest-var
// int 1073741824 = 64^5, i.e. "var slot 5 = 1" on start; ported as an explicit var-5 flag since this
// port's QuestEntry exposes each 6-bit var slot directly instead of one packed int). Use 730702
// (USE_OBJECT, resets all vars back to 0 — Java's qs.setQuestVar(0) decodes 0 into every slot).
// Kill 219354 once (var0==0 -> reward). Turn in at 800067.
...
public sealed class _30702DredgionControlCenterAssault : QuestHandlerBase
{
    private const int QuestIdConst = 30702;
    private const int StartNpc     = 800424;
    private const int UseNpc       = 730702;
    private const int TurnInNpc    = 800067;
    private const int KillNpc      = 219354;

    public _30702DredgionControlCenterAssault(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
