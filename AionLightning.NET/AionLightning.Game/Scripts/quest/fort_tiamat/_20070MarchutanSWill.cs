// Port of Java data/scripts/system/handlers/quest/fort_tiamat/_20070MarchutanSWill.java (Cheatkiller).
// Asmodian counterpart of _10070 (Fort Tiamat instance / world 300490000): 205579 start -> 798800
// (gives token 182213248) -> 205579 (removes token) -> 205864 -> object 730628 (enters instance) ->
// object 730691 (spawns Marchutan 800386) -> 800386 (movie 494, spawns 800387/800431/800358) ->
// 800431 -> 800358 (SET_SUCCEED). Turn in at 205864.
...
public sealed class _20070MarchutanSWill : QuestHandlerBase
{
    private const int QuestIdConst = 20070;
    private const int InstanceWorldId = 300490000;
    private const int TokenItem = 182213248;

    private readonly IItemDao _itemDao;

    public _20070MarchutanSWill(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override ValueTask<bool> OnLevelUpAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
        => DefaultOnLvlUpEventAsync(env, conn, precedingQuestId: 20064, ct);
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        {
            if (targetId != 205864) return false;
        ...
        if (targetId == 800386)
        {
            switch (dialog)
            {
                ...
                case DialogAction.SETPRO7 when var == 6:
                    await PlayQuestMovieAsync(conn, player, 494, ct);
                    // note: Java despawns the talked-to NPC here - no despawn API, dropped (cosmetic).
                    SpawnQuestNpc(InstanceWorldId, player.Position.InstanceId, 800387, 458f, 514f, 417f, 119);
                    SpawnQuestNpc(InstanceWorldId, player.Position.InstanceId, 800431, 502f, 526f, 417f, 70);
                    SpawnQuestNpc(InstanceWorldId, player.Position.InstanceId, 800358, 499f, 500f, 417f, 53);
        ...
        if (targetId == 800358)
        {
            switch (dialog)
            {
                ...
                case DialogAction.SET_SUCCEED when var == 8:
                    // note: Java despawns the talked-to NPC and relocates via TeleportService2 (600030000)
