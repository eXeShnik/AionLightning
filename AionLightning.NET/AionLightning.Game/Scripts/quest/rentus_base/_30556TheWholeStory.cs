// Port of Java data/scripts/system/handlers/quest/rentus_base/_30556TheWholeStory.java (Ritsu).
// Start at Oreitia (799544); hand in the quest_data.xml collect items at the same npc
// (CHECK_USER_HAS_QUEST_ITEM_SIMPLE, no step change, reward=false); then SETPRO1 removes item
// 182212505 and flips var0->1 + reward; turn in at Maios (799549).
...
public sealed class _30556TheWholeStory : QuestHandlerBase
{
    private const int QuestIdConst = 30556;
    private const int StartNpc     = 799544; // Oreitia
    private const int MaiosNpc     = 799549;
    private const int RemoveItemId = 182212505;

    private readonly IItemDao _itemDao;

    public _30556TheWholeStory(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry.Status == QuestStatus.START && targetId == StartNpc)
        {
            ...
            if (dialog == DialogAction.CHECK_USER_HAS_QUEST_ITEM_SIMPLE)
                return await CheckQuestItemsSimpleAsync(env, conn, 0, 0, reward: false, checkOkId: 10000, ct);
