// Port of Java data/scripts/system/handlers/quest/elementis_forest/_30407CatchingtheLight.java (Ritsu).
// Talking to 799551 grants the catching device (182213014); using it converts one device into one
// caught light (182213015, no cap - repeatable); handing in the quest_data.xml collect items (the
// caught lights) at 799551 flips straight to REWARD.
...
public sealed class _30407CatchingtheLight : QuestHandlerBase
{
    private const int QuestIdConst   = 30407;
    private const int StartNpc       = 799551;
    private const int SeerNpc        = 205575;
    private const int DeviceItemId   = 182213014;
    private const int CaughtLightId  = 182213015;

    private readonly IItemDao _itemDao;

    public _30407CatchingtheLight(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override async ValueTask<bool> OnItemUseAsync(Player player, int itemId, GsClientConnection conn, CancellationToken ct)
    {
        ...
        await GiveQuestItemAsync(player, conn, _itemDao, CaughtLightId, 1, ct);
        return true;
    }
