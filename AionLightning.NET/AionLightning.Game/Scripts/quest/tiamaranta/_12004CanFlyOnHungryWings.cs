// Port of Java data/scripts/system/handlers/quest/tiamaranta/_12004CanFlyOnHungryWings.java (Cheatkiller).
// Accept at Kilalu (205897). Talking her again (SETPRO1) spawns Brax the supply carrier (701367) at a
// fixed spot and escorts him to fixed coords (defaultCloseDialog 0->1). Reaching the coords advances
// var0 1->2 (defaultFollowEndEvent 1,2,false); losing him (or dying/logging out at var0 1) reverts 1->0.
// At the destination talk 205948: SET_SUCCEED gives the supply crate 182212590 and flips to REWARD
...
public sealed class _12004CanFlyOnHungryWings : QuestHandlerBase
{
    private const int QuestIdConst = 12004;
    private const int StartNpc  = 205897;
    ...
    public _12004CanFlyOnHungryWings(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        else if (entry.Status == QuestStatus.START)
        {
            if (targetId == StartNpc)
            {
                ...
                {
                    // note: Java sets Brax's walker path (walkerId "12004" + WalkManager) and plays a
                    // START_EMOTE2 emotion — cosmetic movement decoration, dropped; the escort itself is the mechanic.
                    var brax = SpawnQuestNpcAndGet(player.Position.WorldId, player.Position.InstanceId, BraxNpc, 287.21f, 1637.4f, 293.55f, (byte)113);
                    if (brax != null)
                        StartFollowToCoords(env, conn, brax, 733.65125f, 1540.8422f, 733.651f);
