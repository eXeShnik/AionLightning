// Port of Java data/scripts/system/handlers/quest/tiamaranta/_20064Nemesis.java (Cheatkiller).
// Asmodian mirror of _10064TheOldestOfFoes (instance world 300400000): talk 800018 -> SETPRO1 enters
// the instance (var 0->1); on entering the instance world at var 1, decorative NPCs (800035 +
// 800036/800037/800038) spawn; talk 800035 -> SETPRO2 plays movie 755 (var 1->2); movie 755 end spawns
// 13x enemy legioners (800033); clearing all 800033 spawns 6x 800041 + 800021 (var 2->3); talk 800021 ->
// SETPRO3/4 spawns 7 drakans (var 3->4); clearing all drakans spawns boss 218823 (var 4->5); killing
// 218823 plays movie 756 and flips REWARD; turn in at 205886. Leaving the instance world (or dying)
// while in progress reverts the step.
...
public sealed class _20064Nemesis : QuestHandlerBase
{
    private const int QuestIdConst = 20064;
    private const int InstanceWorld = 300400000;
    private const int Mob     = 800033;
    ...
    public _20064Nemesis(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override void Register(QuestEngine engine)
    {
        ...
        engine.RegisterOnQuestMovieEnd(755, QuestId);
        engine.RegisterOnQuestMovieEnd(756, QuestId);
        engine.RegisterQuestNpc(800018).OnTalk.Add(QuestId);
        engine.RegisterQuestNpc(800021).OnTalk.Add(QuestId);
        engine.RegisterQuestNpc(800035).OnTalk.Add(QuestId);
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry.Status == QuestStatus.START)
        {
            ...
            else if (targetId == 800035)
            {
                ...
                {
                    await PlayQuestMovieAsync(conn, player, 755, ct);
    ...
    public override async ValueTask<bool> OnKillAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (var == 2 && targetId == Mob)
        {
            ...
            {
                SpawnQuestNpc(InstanceWorld, instanceId, 800041, 550.057f, 666.941f, 183.301f, 40);
                SpawnQuestNpc(InstanceWorld, instanceId, 800041, 542.284f, 662.375f, 183.301f, 40);
                SpawnQuestNpc(InstanceWorld, instanceId, 800041, 540.240f, 665.980f, 183.301f, 40);
                SpawnQuestNpc(InstanceWorld, instanceId, 800041, 548.020f, 670.540f, 183.301f, 40);
                SpawnQuestNpc(InstanceWorld, instanceId, 800041, 538.181f, 669.566f, 183.301f, 40);
                SpawnQuestNpc(InstanceWorld, instanceId, 800041, 545.943f, 674.044f, 183.301f, 40);
        ...
        {
            await PlayQuestMovieAsync(conn, player, 756, ct);
    ...
    public override async ValueTask<bool> OnMovieEndAsync(QuestEnv env, int movieId, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry.Status == QuestStatus.START && movieId == 755 && entry.GetVar(0) == 2)
        {
            SpawnLegioners(player);
            return true;
        }
        if (entry.Status == QuestStatus.REWARD && movieId == 756)
        {
    ...
    public override async ValueTask<bool> OnEnterWorldAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (player.Position.WorldId == InstanceWorld)
        {
            if (var == 1)
            {
                SpawnQuestNpc(InstanceWorld, instanceId, 800035, 514.004f, 718.839f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800036, 504.51f, 728.05f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800037, 500.42f, 735.26f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800038, 496.2f, 742.43f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800036, 500.91f, 727.45f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800036, 495.33f, 727.25f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800037, 491.35f, 734.78f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800038, 511.76f, 732.18f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800037, 507.68f, 739.07f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800038, 503.49f, 746.21f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800036, 514.64f, 735.77f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800036, 511.53f, 741.44f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800036, 516.96f, 739.95f, 178.393f, 0);
                SpawnQuestNpc(InstanceWorld, instanceId, 800036, 513.035f, 747.17f, 178.393f, 0);
                return true;
            }
        }
        else
