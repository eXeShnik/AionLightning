// Port of Java data/scripts/system/handlers/quest/sarpan/_10053Revelations.java (vlog).
// Zone-mission-chain quest (no OnQuestStart npc - started by 10052's dialog/level-up chain): talk to
// Garnon (205987) var 0->1; entering world 300330000 spawns the Protector's Seal (730493) and flips
// 1->2; use the Seal at var 2 to flip 2->3, start a 180s quest timer, and spawn a random Klaw
// (one of 218760/218761/218762/218763); killing that Klaw (or re-entering the world while var is in
// [2,4)) respawns another; the timer flips 3->4, spawns Oriata (205795) and plays movie 707; hand in
// the quest_data.xml collect-items at Oriata (movie 709 first, at var 5) to flip to REWARD; turn in
// at Killios (205535).
...
public sealed class _10053Revelations : QuestHandlerBase
{
    private const int QuestIdConst        = 10053;
    ...
    public _10053Revelations(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override ValueTask<bool> OnLevelUpAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
        => DefaultOnLvlUpEventAsync(env, conn, precedingQuestId: 10052, isZoneMission: true, ct);
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry.Status == QuestStatus.START)
        {
            ...
            if (targetId == OriataNpc)
            {
                ...
                {
                    await PlayQuestMovieAsync(conn, player, 709, ct);
        ...
        if (entry.Status == QuestStatus.REWARD && targetId == KilliosNpc)
        {
    ...
    public override async ValueTask<bool> OnQuestTimerEndAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        await PlayQuestMovieAsync(conn, player, 707, ct);
        return true;
    }
