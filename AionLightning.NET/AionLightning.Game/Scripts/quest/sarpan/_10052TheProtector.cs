// Port of Java data/scripts/system/handlers/quest/sarpan/_10052TheProtector.java (vlog).
// Zone-mission-chain quest (no OnQuestStart npc - started by 10051's dialog/level-up chain):
// enter SARPAN_CAPITOL_600020000 to flip 0->1 (movie 704); Garnon (205987) var 1->2; Nytia (205788)
// var 2->3; kill Guard (218100) 3->4; enter DEBARIM_PETRALITH_STUDIO_600020000 at var 5 to flip
// 5->6 (movie 705); read Zayedan's Record (730474) 6->7; Ispharel (205988) flips to REWARD; turn
// in at Killios (205535).
// Skip vs Java: Garnon's SETPRO5 branch calls TeleportService2.teleportTo(...) - no TeleportService
// exists in this port, so the interaction is a no-op past that step (the player must walk instead),
// same simplification as sarpan's _10051UnwelcomeMessengers/_20050BuildingBridges.
...
public sealed class _10052TheProtector : QuestHandlerBase
{
    private const int QuestIdConst      = 10052;
    ...
    public _10052TheProtector(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override ValueTask<bool> OnLevelUpAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
        => DefaultOnLvlUpEventAsync(env, conn, precedingQuestId: 10051, isZoneMission: true, ct);
    ...
    public override async ValueTask<bool> OnEnterZoneAsync(QuestEnv env, string zoneName, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (zoneName == DebarimPetralithZone && var == 5)
        {
            ...
            await PlayQuestMovieAsync(conn, env.Player, 705, ct);
            return true;
        }
        return false;
    }
