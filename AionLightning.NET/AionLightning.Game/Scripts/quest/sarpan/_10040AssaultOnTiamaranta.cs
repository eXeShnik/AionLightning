// Port of Java data/scripts/system/handlers/quest/sarpan/_10040AssaultOnTiamaranta.java (zhkchi).
// Elyos instance assault (Eulos, world 300410000). Start at 798926 (var 0->1) -> 800084 SETPRO2 enters
// the instance (772,211,1029) and advances (1->2) -> 799721 SETPRO3 (2->3) -> 730527 USE_OBJECT
// relocates deeper (dropped) -> attack the boss 205818 at var==3: after an 18s scripted sequence the
// var flips 3->4->11 -> 800280 SET_SUCCEED flips to REWARD (relocation to reward world 600020000
// dropped) -> turn in at 205535 in REWARD. Leaving the instance world (onEnterWorld/onDie) or leaving
// the EULOS_CAPTAINS_CABIN/EULOS_DECK zone regions while in progress reverts to var 1.
// Unblocked by RegisterOnLeaveZone/OnLeaveZoneAsync (Java onLeaveZoneEvent) + EnterInstanceAsync
// (Java InstanceService triad); level-up fans out zone-mission-end pokes to 10050-10053 via the
// captured engine (Java QuestEngine.onEnterZoneMissionEnd), mirroring quest/brusthonin/_2091MeettheReapers.
// Skips vs Java (all cosmetic, state transitions preserved): SM_PLAY_MOVIE(1,18) instance-entry
// cutscene; the two TeleportService2 relocations (730527 in-instance hop, 800280 reward-world hop);
...
public sealed class _10040AssaultOnTiamaranta : QuestHandlerBase
{
    private const int QuestIdConst = 10040;
    private const int StartNpc      = 798926;
    private const int Npc800084     = 800084;
    private const int Npc799721     = 799721;
    private const int Npc730527     = 730527;
    private const int Npc800280     = 800280;
    private const int TurnInNpc     = 205535;
    private const int BossMob       = 205818;
    private const int InstanceWorld = 300410000;
    private const int RewardWorld   = 600020000;
    private const string ZoneCabin  = "EULOS_CAPTAINS_CABIN_300410000";
    private const string ZoneDeck   = "EULOS_DECK_300410000";

    private static readonly int[] _zoneMissions = [10050, 10051, 10052, 10053];

    private QuestEngine? _engine;

    public _10040AssaultOnTiamaranta(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override void Register(QuestEngine engine)
    {
        ...
        engine.RegisterQuestNpc(Npc800084).OnTalk.Add(QuestId);
        engine.RegisterQuestNpc(Npc799721).OnTalk.Add(QuestId);
        engine.RegisterQuestNpc(Npc800280).OnTalk.Add(QuestId);
        engine.RegisterQuestNpc(TurnInNpc).OnTalk.Add(QuestId);
        engine.RegisterQuestNpc(Npc730527).OnTalk.Add(QuestId);
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry.Status == QuestStatus.START)
        {
            ...
            if (targetId == Npc800084 && var == 1)
            {
                if (dialog == DialogAction.QUEST_SELECT)
                    return await SendQuestDialogAsync(conn, targetObjId, 1352, ct);
                if (dialog == DialogAction.SETPRO2)
                {
                    await EnterInstanceAsync(player, conn, InstanceWorld, 772f, 211f, 1029f, 0, ct);
                    await ChangeQuestStepAsync(conn, entry, 0, 2, toReward: false, ct);
                    // note: Java plays SM_PLAY_MOVIE(1, 18) instance-entry cutscene here — cosmetic movie broadcast, dropped.
                    return await CloseDialogWindowAsync(conn, targetObjId, ct);
                }
                return await SendQuestStartDialogAsync(env, conn, ct);
            }
            if (targetId == Npc799721 && var == 2)
            {
                if (dialog == DialogAction.QUEST_SELECT)
            ...
            if (targetId == Npc730527 && var == 3)
            {
                if (dialog == DialogAction.USE_OBJECT)
                {
                    // note: Java relocates within the instance (300410000, 756,214,1029, heading 116, BEAM) — non-entry relocation, dropped; return true kept.
                    return true;
                }
                return await SendQuestStartDialogAsync(env, conn, ct);
            }
            if (targetId == Npc800280)
            {
                ...
                if (dialog == DialogAction.SET_SUCCEED)
                {
                    ...
                    await UpdateQuestStatusAsync(conn, entry, ct);
                    // note: Java teleports to reward world 600020000 (1219,1361,1359, heading 70, BEAM) here — non-entry relocation, dropped.
                    return await CloseDialogWindowAsync(conn, targetObjId, ct);
                }
                return await SendQuestStartDialogAsync(env, conn, ct);
            }
    ...
    public override async ValueTask<bool> OnLeaveZoneAsync(QuestEnv env, string zoneName, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (zoneName == ZoneCabin)
        {
            ...
            {
                entry.SetVar(0, 1);
                // Java bug: _10040 omits updateQuestStatus here (its twin _20040 includes it); added so the step revert persists and broadcasts.
                await UpdateQuestStatusAsync(conn, entry, ct);
                return true;
            }
        }
        ...
        {
            entry.SetVar(0, 1);
            // Java bug: same omission as the cabin branch; added for consistency with the twin _20040.
            await UpdateQuestStatusAsync(conn, entry, ct);
            return true;
        }
        return false;
