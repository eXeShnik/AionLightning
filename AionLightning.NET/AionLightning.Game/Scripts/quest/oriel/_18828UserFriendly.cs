// Port of Java data/scripts/system/handlers/quest/oriel/_18828UserFriendly.java (Rolandas).
// Talk to any of 5 butler-template npcs (810017-810021) to accept — the accept dialog gives item
// 182213191 x1 (Java's item-overload sendQuestStartDialog(env, itemId, count): aborts the accept if
// the bag can't fit it, same guard as _18808FoolproofPackaging.cs); using the item while START flips
// straight to REWARD (var 0->1) via the item-use trigger (Batch 0.1 OnItemUse hook); turn in at the
// butler, which removes the item on SELECT_QUEST_REWARD before showing the end dialog — Java's
// switch falls through from SELECT_QUEST_REWARD into SELECTED_QUEST_NOREWARD's body with no
// `break`, ported 1:1 via the explicit OR below (same documented pattern as
// pandaemonium/_2937UnexpectedReward.cs).
...
public sealed class _18828UserFriendly : QuestHandlerBase
{
    private const int QuestIdConst = 18828;
    private const int GiftItemId   = 182213191;

    private static readonly HashSet<int> Butlers = [810017, 810018, 810019, 810020, 810021];

    private readonly IItemDao _itemDao;

    public _18828UserFriendly(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
