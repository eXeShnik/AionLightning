// Port of Java data/scripts/system/handlers/quest/rider_quests/_24031EnemyAtTheDoorstep.java.
// Morheim chain (level-up gated on 24030): Vidar (204052, var0 0->1), Rita (801224, 1->2), Munin
// (203550, 2->3 giving item 182215394), then USE 182215394 inside DF1_ITEMUSEAREA_Q24031 (3->4),
// Autri (203654, 4->5 giving 182215395), USE 182215395 inside DF1A_ITEMUSEAREA_Q24031 (5->6), Tyr
// (204369, 6->7 giving 182215396), USE 182215396 inside DF2_ITEMUSEAREA_Q24031 (7->8), Tyr again
// (8->9) enters instance world 320040000 and spawns boss 233879; kill 233879 (var0==9) spawns the
// Large Teleport (730888); USE it (var0==10) spawns the Shattered Large Teleport (730898) and
// advances 10->11; USE that (var0==11) flips to REWARD. Turn in at Vidar. Dying or leaving 320040000
// while var0 in [9,11) reverts to var0 8.
// Instance entry uses EnterInstanceAsync; in-instance/world spawns use SpawnQuestNpc; player-death
// revert uses OnDieAsync; logout revert uses OnLogOutAsync (persists without packets).
// Skips vs Java: (1) SM_ITEM_USAGE_ANIMATION cast-time animation + 3s schedule on each item-use
// dropped — the item removal and var++ apply immediately. (2) npc.getController().onDelete() despawns
// of the teleport objects dropped — no despawn API. (3) TeleportService2 relocation to Morheim
// (220020000) on the final USE dropped — non-entry relocation, the REWARD flip is kept.
// (4) QUEST_FAILED_$1 system message dropped — cosmetic notice.
...
public sealed class _24031EnemyAtTheDoorstep : QuestHandlerBase
{
    private const int QuestIdConst = 24031;
    private const int VidarNpc     = 204052;
    private const int RitaNpc      = 801224;
    private const int MuninNpc     = 203550;
    private const int AutriNpc     = 203654;
    private const int TyrNpc       = 204369;
    private const int LargeTele    = 730888;
    private const int ShatteredTele = 730898;
    private const int BossNpc      = 233879;
    private const int Item1        = 182215394;
    private const int Item2        = 182215395;
    private const int Item3        = 182215396;
    private const int InstanceWorld = 320040000;
    private const string Zone1     = "DF1_ITEMUSEAREA_Q24031";
    private const string Zone2     = "DF1A_ITEMUSEAREA_Q24031";
    private const string Zone3     = "DF2_ITEMUSEAREA_Q24031";

    private readonly IItemDao _itemDao;

    public _24031EnemyAtTheDoorstep(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override void Register(QuestEngine engine)
    {
        ...
        foreach (int npc in new[] { VidarNpc, RitaNpc, MuninNpc, AutriNpc, TyrNpc, LargeTele, ShatteredTele })
    ...
    public override ValueTask<bool> OnLevelUpAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
        => DefaultOnLvlUpEventAsync(env, conn, 24030, isZoneMission: true, ct);
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        {
            if (targetId == VidarNpc) return await SendQuestEndDialogAsync(env, conn, ct);
            return false;
        }
        ...
        if (targetId == ShatteredTele)
        {
            if (dialog == DialogAction.USE_OBJECT && var == 11)
            {
                ...
                // note: npc.getController().onDelete() despawn + TeleportService2 relocation to Morheim (220020000) dropped — non-entry relocation; REWARD flip kept
                return true;
            }
        }
        return false;
