// Port of Java data/scripts/system/handlers/quest/morheim/_2436LookingForBuBuPat.java (Cheatkiller).
// Accept at 204390. Talking BuBuPat (204401, SETPRO1) makes it follow the player to 204390
// (var0 0->1); reaching there flips to REWARD (defaultFollowEndEvent 1,1,true). Report to 204390.
...
public sealed class _2436LookingForBuBuPat : QuestHandlerBase
{
    private const int QuestIdConst = 2436;
    private const int StartNpc     = 204390;
    private const int FollowerNpc  = 204401;

    public _2436LookingForBuBuPat(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
        ...
        else if (entry.Status == QuestStatus.START)
        {
            if (targetId == FollowerNpc)
            {
                ...
                else if (dialog == DialogAction.SETPRO1)
                {
                    ...
                    StartFollowToNpc(env, conn, follower, StartNpc);
