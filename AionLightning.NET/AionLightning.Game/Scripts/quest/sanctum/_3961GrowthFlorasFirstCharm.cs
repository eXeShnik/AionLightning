// Port of Java data/scripts/system/handlers/quest/sanctum/_3961GrowthFlorasFirstCharm.java (undertrey / vlog).
// Talk to Flora (798384) to start; Erdos (203740) hands over a charm-request slip (item
// 182206108, var 0->1); bring it back to Flora with 40000 kinah to advance to REWARD.
...
public sealed class _3961GrowthFlorasFirstCharm : QuestHandlerBase
{
    private const int QuestIdConst = 3961;
    private const int FloraNpc     = 798384;
    private const int ErdosNpc     = 203740;
    private const int RequestItemId = 182206108;
    private const int KinahCost    = 40000;
    private const int KinahItemId  = 182400001;

    private readonly IItemDao _itemDao;

    public _3961GrowthFlorasFirstCharm(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry.Status == QuestStatus.START)
        {
            ...
            else if (targetId == FloraNpc)
            {
                ...
                {
                    if (var == 1 && await TryDecreaseKinahAsync(player, conn, KinahCost, ct))
                    {
