// Port of Java data/scripts/system/handlers/quest/sanctum/_3962GrowthFlorasSecondCharm.java (undertrey / vlog).
// Talk to Flora (798384) to start; Erdos (203740) hands over a charm-request slip (item
// 182206109, var 0->1); bring it back to Flora with a Blue Aether Powder (186000088) and 50000
// kinah to advance to REWARD. Java bug fix: the original decreased kinah first and checked the
// powder count second (`&&` short-circuit order), so a player without the powder still lost the
// kinah; this port checks the powder count before spending kinah.
...
public sealed class _3962GrowthFlorasSecondCharm : QuestHandlerBase
{
    private const int QuestIdConst = 3962;
    private const int FloraNpc     = 798384;
    private const int ErdosNpc     = 203740;
    private const int RequestItemId = 182206109;
    private const int PowderItemId  = 186000088;
    private const int KinahCost    = 50000;
    private const int KinahItemId  = 182400001;

    private readonly IItemDao _itemDao;

    public _3962GrowthFlorasSecondCharm(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
