// Port of Java data/scripts/system/handlers/quest/event_quests/_80018EventSockItToEm.java (npc 799778).
// Identical shape to _80016EventSockHop, different npc and movie ids (135/136).
...
public sealed class _80018EventSockItToEm : QuestHandlerBase
{
    private const int QuestIdConst = 80018;
    private const int Npc          = 799778;

    private readonly IItemDao _itemDao;

    public _80018EventSockItToEm(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override async ValueTask<HandlerResult> OnBonusApplyAsync(QuestEnv env, string bonusType, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry is { Status: QuestStatus.REWARD })
        {
            ...
            if (Random.Shared.Next(100) < 50)
                await PlayQuestMovieAsync(conn, player, 135, ct);
            else
                await PlayQuestMovieAsync(conn, player, 136, ct);
    ...
    // Java sendQuestRewardDialog(env, 799778, 0): reportDialogId 0 means a REWARD-status turn-in
