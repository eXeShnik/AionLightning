// Port of Java data/scripts/system/handlers/quest/event_quests/_80028EventMeetTheFayrefolk.java.
// Single-npc accept/turn-in quest at Fay (799766) — same shape as the already-ported
// _80029EventUsingYourCharms.cs (no NONE-status handling; the npc is also registered OnQuestStart,
// so the generic engine fallback in CM_DIALOG_SELECT offers/starts the quest via quest_data.xml).
...
public sealed class _80028EventMeetTheFayrefolk : QuestHandlerBase
{
    private const int QuestIdConst = 80028;
    private const int FayNpc       = 799766;

    public _80028EventMeetTheFayrefolk(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    /// <summary>Java sendQuestRewardDialog(env, 799766, reportDialogId): a REWARD-status turn-in
    /// shows <paramref name="reportDialogId"/> on USE_OBJECT (when non-zero), else finishes the quest.</summary>
