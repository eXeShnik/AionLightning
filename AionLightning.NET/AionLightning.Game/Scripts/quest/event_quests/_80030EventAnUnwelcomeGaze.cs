// Port of Java data/scripts/system/handlers/quest/event_quests/_80030EventAnUnwelcomeGaze.java (Elyos, npc 799766).
// note: Java EventService.checkQuestIsActive (global event-toggle config) is not ported; the IsQuestActive
//   base helper reinterprets it as "player has a non-COMPLETE entry", so the parent-quest gate and the
//   level-up abandon branch follow that mapping rather than a server event switch.
...
public sealed class _80030EventAnUnwelcomeGaze : QuestHandlerBase
{
    private const int QuestIdConst = 80030;
    private const int Npc          = 799766;
    private const int ParentQuest  = 80029;
    private const int CharmCard     = 188051133; // [Event] Charm Card
    private const int BeritrasGaze  = 164002015;

    private readonly IItemDao _itemDao;

    public _80030EventAnUnwelcomeGaze(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    {
        // Java: only proceed when the parent quest (which awards the Charm Cards) is active.
        if (!IsQuestActive(player, ParentQuest)) return false; // Java HandlerResult.UNKNOWN

        // The same Charm Card item is registered for both the Elyos (this quest) and Asmodian (80033)
        // handlers; gate on race so we only start the Elyos chain here.
        if (itemId == CharmCard && player.Race == Race.ELYOS)
        {
            ...
            // note: Java also (re)starts the sibling Elyos event quests 80034/80035/80036 here (each when
    ...
    public override async ValueTask<bool> OnLevelUpAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        var player = env.Player;
        var entry = player.Quests.Get(QuestId);
        if (!IsQuestActive(player, QuestId) && entry is not null)
            await AbandonQuestAsync(conn, player, ct);
        return true; // Java onLvlUpEvent returns true unconditionally
    }
    ...
    // Java sendQuestRewardDialog(env, 799766, reportDialogId): a REWARD-status turn-in shows
