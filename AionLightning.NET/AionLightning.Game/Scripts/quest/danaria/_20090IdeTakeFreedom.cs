// Port of Java data/scripts/system/handlers/quest/danaria/_20090IdeTakeFreedom.java (pralinka).
// Asmodian mirror of _10090IdgelIfYouAskMe (world 301000000). Paut (800567, var 0->1) -> Luciana
// (800821, 1->2) -> strange cube (730736) enters the instance, spawns Lucullus1 (800822), plays
// movie 852 (2->3) -> Lucullus1 (3->4) -> kill rusted door (701545, 4->5) -> enter
// QUEST_20090_301000000 zone (5->6) -> Danuar shield (730735) at var 6 starts a 120s timer, spawns
// four adds (231067 x2 / 231069 x2) (6->7); the timer end spawns Lucullus2 (800823) and advances
// 7->8 -> Lucullus2 SET_SUCCEED flips to REWARD. Turn in at 800825. Unblocked by EnterInstanceAsync
// + OnDieAsync/OnLogOutAsync.
...
public sealed class _20090IdeTakeFreedom : QuestHandlerBase
{
    private const int QuestIdConst = 20090;
    private const int InstanceWorldId = 301000000;
    private const string EnterZoneName = "QUEST_20090_301000000";

    public _20090IdeTakeFreedom(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override ValueTask<bool> OnLevelUpAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
        => DefaultOnLvlUpEventAsync(env, conn, 20085, ct);
    ...
    public override async ValueTask<bool> OnQuestTimerEndAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        SpawnQuestNpc(InstanceWorldId, player.Position.InstanceId, 800823, 564f, 426f, 95f, 119);
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry.Status == QuestStatus.START)
        {
            ...
            if (targetId == 730736) // strange cube
            {
                ...
                if (dialog == DialogAction.SETPRO3)
                {
                    ...
                    SpawnQuestNpc(InstanceWorldId, player.Position.InstanceId, 800822, 210f, 507f, 154f, 119);
                    await PlayQuestMovieAsync(conn, player, 852, ct);
            ...
            if (targetId == 800823) // lucullus2
            {
                if (dialog == DialogAction.QUEST_SELECT && var == 8)
                    return await SendQuestDialogAsync(conn, targetObjId, 3398, ct);
                if (dialog == DialogAction.SET_SUCCEED)
                {
                    // note: Java teleports to 600060000 (1399,350,307) here - non-entry relocation, dropped.
        ...
        if (entry.Status == QuestStatus.REWARD && targetId == 800825)
        {
