// Port of Java data/scripts/system/handlers/quest/danaria/_13356HomerusRequest.java (Romanz).
// Auto-starts on entering Danaria's SILONA_FOUNTAINHEAD_600060000 region (registerOnEnterZone -> QuestService.startQuest);
...
// player.isInsideZone(SILONA_FOUNTAINHEAD_600060000) — no zone-shape infra in this port, so the trigger is broadened to
// "anywhere in the world" like every other kill_in_world port here, e.g. sarpan's
// _11522FurbackHunting); turn in at HomerusRequest (801042). qs.canRepeat() (daily-reset re-entry) isn't
...
public sealed class _13356HomerusRequest : QuestHandlerBase
{
    private const int QuestIdConst   = 13356;
    private const int TurnInNpc      = 801042;
    private const int DanariaWorldId = 600060000;
    private const string EnterZoneName = "SILONA_FOUNTAINHEAD_600060000";

    public _13356HomerusRequest(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
