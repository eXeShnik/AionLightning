// Port of Java data/scripts/system/handlers/quest/danaria/_23351TobaldRequest.java (Romanz).
// Auto-starts on entering Danaria's SLEEPING_ELDERS_PLATEAU_600060000 region (registerOnEnterZone -> QuestService.startQuest);
...
// player.isInsideZone(SLEEPING_ELDERS_PLATEAU_600060000) — no zone-shape infra in this port, so the trigger is broadened to
// "anywhere in the world" like every other kill_in_world port here, e.g. sarpan's
// _11522FurbackHunting); turn in at TobaldRequest (801047). qs.canRepeat() (daily-reset re-entry) isn't
...
public sealed class _23351TobaldRequest : QuestHandlerBase
{
    private const int QuestIdConst   = 23351;
    private const int TurnInNpc      = 801047;
    private const int DanariaWorldId = 600060000;
    private const string EnterZoneName = "SLEEPING_ELDERS_PLATEAU_600060000";

    public _23351TobaldRequest(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
