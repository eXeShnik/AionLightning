// Port of Java data/scripts/system/handlers/quest/danaria/_13353PerianderCallsForAid.java (Romanz).
// Auto-starts on entering Danaria's SILENT_HILLS_600060000 region (registerOnEnterZone -> QuestService.startQuest);
...
// player.isInsideZone(SILENT_HILLS_600060000) — no zone-shape infra in this port, so the trigger is broadened to
// "anywhere in the world" like every other kill_in_world port here, e.g. sarpan's
// _11522FurbackHunting); turn in at PerianderCallsForAid (801039). qs.canRepeat() (daily-reset re-entry) isn't
...
public sealed class _13353PerianderCallsForAid : QuestHandlerBase
{
    private const int QuestIdConst   = 13353;
    private const int TurnInNpc      = 801039;
    private const int DanariaWorldId = 600060000;
    private const string EnterZoneName = "SILENT_HILLS_600060000";

    public _13353PerianderCallsForAid(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
