// Port of Java data/scripts/system/handlers/quest/danaria/_13355KalennosRequest.java (Romanz).
// Auto-starts on entering Danaria's DRAINED_BASIN_600060000 region (registerOnEnterZone -> QuestService.startQuest);
...
// player.isInsideZone(DRAINED_BASIN_600060000) — no zone-shape infra in this port, so the trigger is broadened to
// "anywhere in the world" like every other kill_in_world port here, e.g. sarpan's
// _11522FurbackHunting); turn in at KalennosRequest (801041). qs.canRepeat() (daily-reset re-entry) isn't
...
public sealed class _13355KalennosRequest : QuestHandlerBase
{
    private const int QuestIdConst   = 13355;
    private const int TurnInNpc      = 801041;
    private const int DanariaWorldId = 600060000;
    private const string EnterZoneName = "DRAINED_BASIN_600060000";

    public _13355KalennosRequest(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
