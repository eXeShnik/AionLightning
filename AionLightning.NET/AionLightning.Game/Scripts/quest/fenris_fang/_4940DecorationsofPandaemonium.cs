// Port of Java data/scripts/system/handlers/quest/fenris_fang/_4940DecorationsofPandaemonium.java
// (Gigi). Single-npc quest at Tragi (204050): accept, then hand in 10x each of four decoration
// items (182207117/118/119/120) to flip straight to reward; repeatable (NONE and COMPLETE both
// re-show the start dialog, matching Java's status guard).
...
public sealed class _4940DecorationsofPandaemonium : QuestHandlerBase
{
    private const int QuestIdConst = 4940;
    private const int TragiNpc     = 204050;
    private const int Decoration1  = 182207117;
    private const int Decoration2  = 182207118;
    private const int Decoration3  = 182207119;
    private const int Decoration4  = 182207120;
    private const int RequiredEach = 10;

    private readonly IItemDao _itemDao;

    public _4940DecorationsofPandaemonium(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    {
        engine.RegisterQuestNpc(TragiNpc).OnQuestStart.Add(QuestId);
        engine.RegisterQuestNpc(TragiNpc).OnTalk.Add(QuestId);
    }
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (targetId != TragiNpc) return false;
