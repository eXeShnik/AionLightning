// Port of Java data/scripts/system/handlers/quest/fenris_fang/_4944LoyaltyAndAffableness.java
// (vlog). Start at Kvasir (204053, no starter item); a quest_data.xml collect-item check (amulets)
// advances var 0->6; killing any of six Great Protectors (251002/251021/251018/251039/251033/
// 251036) increments the RAW packed quest-var counter one at a time from 6 up to 306 (Java
...
// which only touches the low slot and would corrupt the count once it crosses 64); back at Kvasir,
// dialog SETPRO3 resets the packed var to 3 (qs.setQuestVar(3)); killing either Dredgion captain
// (214823/216850) while var==3 advances 3->4 (var-slot 0 write here, matching Java's
// defaultOnKillEvent/changeQuestStep, which always operate on slot 0 regardless of how "var" was
// read for display); Balder (204075) requires 4000+ DP, then 186000087 x1, to flip to reward
// (Java's plain checkItemExistence, reimplemented inline); turn in at Kvasir. Java's QUEST_SELECT
// case at Kvasir falls through (no break) into CHECK_USER_HAS_QUEST_ITEM when none of the var==0/
// 306/4 branches match — reproduced with an explicit `goto case`.
// Java bug preserved: the REWARD-status branch at Kvasir checks QUEST_SELECT (not USE_OBJECT, unlike
// every sibling quest in this zone) before falling back to sendQuestEndDialog — kept as authored.
using System.Linq;
...
public sealed class _4944LoyaltyAndAffableness : QuestHandlerBase
{
    private const int QuestIdConst = 4944;
    private const int KvasirNpc    = 204053;
    private const int BalderNpc    = 204075;
    private const int HolyWaterItem = 186000087;
    ...
    public _4944LoyaltyAndAffableness(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
            ...
            if (targetId == BalderNpc)
            {
                switch (dialog)
                {
                    ...
                    case DialogAction.SELECT_ACTION_2718:
                        if (player.Dp >= 4000)
                            return await CheckItemExistenceAsync(env, conn, 5, 5, reward: false, HolyWaterItem, 1, remove: true, 2718, 2887, 0, 0, ct);
