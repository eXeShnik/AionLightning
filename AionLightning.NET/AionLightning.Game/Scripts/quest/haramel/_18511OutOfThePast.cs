// Port of Java data/scripts/system/handlers/quest/haramel/_18511OutOfThePast.java.
// Broth-brewing quest: accept from Moorilerk (799522); loot Well-Dried Ginseng (700954, stub —
// no reward-drop system ported), use the Huge Cauldron (730359) to check collected quest items
// and brew the broth (182212011); receiving the broth item auto-advances var 0->1 and flips the
// quest to REWARD (Java's onGetItemEvent hook); turn in at 790000.
using System.Linq;
...
public sealed class _18511OutOfThePast : QuestHandlerBase
{
    private const int QuestIdConst        = 18511;
    ...
    private const int TurnInNpc           = 790000;
    private const int BrothItemId         = 182212011;

    private readonly IItemDao _itemDao;

    public _18511OutOfThePast(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override void Register(QuestEngine engine)
    {
        ...
        engine.RegisterItemGet(BrothItemId, QuestId);
        foreach (int npc in new[] { MoorilerkNpc, WellDriedGinsengNpc, HugeCauldronObj, TurnInNpc })
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry.Status == QuestStatus.START)
        {
            ...
            if (targetId == HugeCauldronObj)
            {
                ...
                {
                    await GiveQuestItemAsync(player, conn, _itemDao, BrothItemId, 1, ct);
    ...
    {
        if (itemId != BrothItemId) return false;
        var entry = player.Quests.Get(QuestId);
