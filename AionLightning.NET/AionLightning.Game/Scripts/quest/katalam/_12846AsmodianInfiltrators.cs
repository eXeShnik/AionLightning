// Port of Java data/scripts/system/handlers/quest/katalam/_12846AsmodianInfiltrators.java (Romanz).
// Auto-starts on entering Katalam's BERITRAN_FOOTHOLD_600050000 region (registerOnEnterZone ->
...
// additionally required player.isInsideZone(BERITRAN_FOOTHOLD_600050000) — no zone-shape infra in
// this port, so the trigger is broadened to "anywhere in the world", matching every other
// kill_in_world port here, e.g. danaria's _23350HervoRequest); turn in at AsmodianInfiltrators npc
// (801232). qs.canRepeat() (daily-reset re-entry) isn't ported, matching every other kill_in_world
// quest already in this codebase.
...
public sealed class _12846AsmodianInfiltrators : QuestHandlerBase
{
    private const int QuestIdConst   = 12846;
    private const int TurnInNpc      = 801232;
    private const int KatalamWorldId = 600050000;
    private const string EnterZoneName = "BERITRAN_FOOTHOLD_600050000";

    public _12846AsmodianInfiltrators(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
