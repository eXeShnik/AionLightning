// Port of Java data/scripts/system/handlers/quest/katalam/_10082AReianSecret.java (tyrto).
// Elyos chain 801231 -> 800532 -> 800533 (collect) -> onAtDistance near 800534 (movie 823, var 3->4)
// -> 800534 (SET_SUCCEED -> reward) -> turn in 800535. Unblocked by the new onAtDistance hook.
...
public sealed class _10082AReianSecret : QuestHandlerBase
{
    private const int QuestIdConst = 10082;

    private readonly IItemDao _itemDao;

    public _10082AReianSecret(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override void Register(QuestEngine engine)
    {
        ...
        foreach (int npcId in new[] { 801231, 800532, 800533, 800534, 800535 })
            engine.RegisterQuestNpc(npcId).OnTalk.Add(QuestId);
    }

    public override ValueTask<bool> OnLevelUpAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
        => DefaultOnLvlUpEventAsync(env, conn, precedingQuestId: 10081, ct);
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry is not null && entry.Status == QuestStatus.START)
        {
            ...
            if (targetId == 800534)
            {
                ...
                // Java switch fallthrough: QUEST_SELECT (var!=5) falls into SET_SUCCEED
        ...
        else if (entry is not null && entry.Status == QuestStatus.REWARD && targetId == 800535)
        {
    ...
    public override async ValueTask<bool> OnAtDistanceAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry.GetVar(0) == 3)
        {
            ...
            await PlayQuestMovieAsync(conn, env.Player, 823, ct);
            return true;
        }
        return false;
    }
