// Port of Java data/scripts/system/handlers/quest/katalam/_10084SurviveByTheSword.java (tyrto, apozema).
// Long talk/kill chain across Torio(800548) -> kills 230407/230408 -> Jemini(800549 gives item) ->
// use item (var 8->9) -> Gram(800552) -> Beritra remains(800550) -> onAtDistance near 206284/231225
// (var 11->12) -> Thrasir(800561) -> Broken Sword(701538) -> turn in Jakahn(800541).
...
public sealed class _10084SurviveByTheSword : QuestHandlerBase
{
    private const int QuestIdConst = 10084;
    private static readonly int[] Mobs = { 230407, 230408 };

    private readonly IItemDao _itemDao;

    public _10084SurviveByTheSword(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    {
        engine.RegisterQuestItem(182215224, QuestId);
        RegisterOnAtDistance(engine, 206284);
        RegisterOnAtDistance(engine, 231225);
        engine.RegisterOnLevelUp(QuestId);
        foreach (int npcId in new[] { 800548, 800549, 800552, 800550, 800561, 701538, 800541 })
    ...
    public override ValueTask<bool> OnLevelUpAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
        => DefaultOnLvlUpEventAsync(env, conn, precedingQuestId: 10083, ct);
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (entry is not null && entry.Status == QuestStatus.START)
        {
            ...
            if (targetId == 800549) // Jemini
            {
                ...
                {
                    await GiveQuestItemAsync(player, conn, _itemDao, 182215224, 1, ct);
                    return await DefaultCloseDialogAsync(env, conn, 7, 8, ct);
                }
                return false;
            }
            if (targetId == 800552) // Gram
            {
            ...
            if (targetId == 800550) // Mortal remains of a Beritra soldier
            {
                ...
                {
                    await GiveQuestItemAsync(player, conn, _itemDao, 182215228, 1, ct);
                    return await DefaultCloseDialogAsync(env, conn, 10, 11, ct);
                }
                return false;
            }
            if (targetId == 800561) // Thrasir
            {
            ...
            if (targetId == 701538) // Broken Sword
            {
                ...
                {
                    await GiveQuestItemAsync(player, conn, _itemDao, 182215229, 1, ct);
                    return await DefaultCloseDialogAsync(env, conn, 13, 14, reward: true, sameNpc: false, ct);
                }
                return false;
            }
        }
        else if (entry is not null && entry.Status == QuestStatus.REWARD && targetId == 800541) // Jakahn
        {
            if (dialog == DialogAction.USE_OBJECT)
                return await SendQuestDialogAsync(conn, targetObjId, 5, ct);
            await RemoveQuestItemAsync(player, conn, _itemDao, 182215229, 1, ct);
            await RemoveQuestItemAsync(player, conn, _itemDao, 182215228, 1, ct);
