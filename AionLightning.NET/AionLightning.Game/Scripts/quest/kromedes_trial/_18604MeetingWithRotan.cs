// Port of Java data/scripts/system/handlers/quest/kromedes_trial/_18604MeetingWithRotan.java (Rolandas).
// Auto-starts on entering the GRAND_CAVERN_300230000 region; using the Grave Robber's Corpse (700961,
// USE_OBJECT) while START flips to REWARD and finishes; otherwise it hands out lore item 164000141
// (dialog 1012) or reports it already held (dialog 27).
// Skip vs Java: onCanAct (allow ACTION_ITEM_USE on the corpse even after completion) has no hook in
// this port — the corpse is registered for OnTalk so its dialog is dispatched regardless, so the
...
public sealed class _18604MeetingWithRotan : QuestHandlerBase
{
    private const int QuestIdConst     = 18604;
    ...
    public _18604MeetingWithRotan(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
