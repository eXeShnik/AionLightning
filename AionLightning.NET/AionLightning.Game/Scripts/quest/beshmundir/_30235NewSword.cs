// Port of Java data/scripts/system/handlers/quest/beshmundir/_30235NewSword.java (vlog).
// Requires owning the Noble Siel's Supreme Sword (100000939) to start at Gefeios (799032); kill any
// of the three Debilkarim (286904/281419/215795) while all four quest_data.xml collect_items (base
// weapon + essences 186000099/186000106/186000107) are held to consume them and grant the
// quest-work token (182209633), flipping straight to REWARD; turn in at Gefeios.
// Java bug/skip: Java's registerGetingItem/onGetItemEvent flips to REWARD only when the token item
// is *acquired* (fired synchronously by Java's ItemService on any add) - this port's
// GiveQuestItemAsync doesn't dispatch OnItemGetAsync (see eltnen/_1037SecretsoftheTemple for the
// established precedent), so the reward flip is applied directly alongside the kill-time give;
// OnItemGetAsync is still registered/implemented for parity in case the token is ever acquired
// through another source (e.g. direct loot).
using System.Linq;
...
public sealed class _30235NewSword : QuestHandlerBase
{
    private const int QuestIdConst = 30235;
    private const int StartNpc     = 799032; // Gefeios
    private const int BaseWeaponItemId = 100000939; // Noble Siel's Supreme Sword
    private const int QuestWorkItemId  = 182209633;
    private static readonly int[] DebilkarimNpcIds = [286904, 281419, 215795];

    private readonly IItemDao _itemDao;

    public _30235NewSword(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    /// <summary>Checks and consumes all of this quest's quest_data.xml collect_items (Java QuestService.collectItemCheck(env, true)).</summary>
