// Port of Java data/scripts/system/handlers/quest/pandaemonium/_4966GrowthNinissFirstCharm.java.
// Accept default at Ninis (798385); Maochinicherk (798068) gives the Charm Certificate
// (182207136); back at Ninis, paying 40000 kinah flips to REWARD; turn in at Ninis. The two Java
// switch statements each have a case falling through without a break (QUEST_SELECT -> next case)
// — dead in practice once var has advanced past the guarded value, ported via the explicit ORs
// below rather than reproduced as real fallthrough.
...
public sealed class _4966GrowthNinissFirstCharm : QuestHandlerBase
{
    private const int QuestIdConst = 4966;
    private const int NinisNpc = 798385;
    private const int MaochinicherkNpc = 798068;
    private const int CertificateItem = 182207136;
    private const long KinahCost = 40000;
    private const int KinahItemId = 182400001;

    private readonly IItemDao _itemDao;

    public _4966GrowthNinissFirstCharm(IDataManager dataManager, IQuestDao questDao, QuestRewardService rewardService, IItemDao itemDao)
    ...
    public override async ValueTask<bool> OnDialogAsync(QuestEnv env, GsClientConnection conn, CancellationToken ct)
    {
        ...
        if (targetId == NinisNpc)
        {
            ...
            {
                if (var == 1 && await TryDeductKinahAsync(player, conn, KinahCost, ct))
                {
